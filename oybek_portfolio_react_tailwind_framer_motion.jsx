/*
  Oybek Ilhomboyev Portfolio - React (Tailwind + Framer Motion)

  FIXED: removed static import of profile image (which caused build failure when file
  wasn't present). Instead this component now uses a runtime public-path image
  (/profile.png) with a graceful SVG-data fallback if the image is missing.

  ✅ What you must do when running locally:
    1. Put your image file into the React project's public folder and name it
         `public/profile.png`
       (If you prefer another name, update the initialImg variable below.)
    2. Start the dev server (`npm run dev` or `npm start`). No build-time image import
       errors will occur because the image is loaded at runtime.

  Technical notes / fixes included:
    - Replaced boolean "scrolled" state with numeric scrollY to support both
      header threshold and parallax calculation.
    - Image uses onError handler and falls back to an inline SVG (data URI)
      so missing image doesn't break the app.
    - Kept Framer Motion + IntersectionObserver reveal animations and dark-mode
      functionality. Tailwind `dark:` classes are used—make sure Tailwind is
      configured to scan your files.
*/
...
export default function Portfolio() {
  ...
  // Image handling: runtime public path + SVG fallback
  // Drop your image at `public/profile.png` (or change initialImg below)
  const initialImg = "/profile.png"; // runtime public path (safe for build)

  const FALLBACK_SVG = encodeURIComponent(`
    <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 200 200'>
      <rect width='100%' height='100%' rx='28' fill='#4338CA' />
      <text x='50%' y='55%' font-size='64' fill='white' text-anchor='middle' font-family='Inter, Arial' font-weight='700'>OI</text>
    </svg>
  `);
  ...
  // Header threshold and parallax value
  const headerSolid = scrollY > 60; // boolean for header background
  const parallax = Math.min(scrollY / 6, 40); // limited parallax translate

  return (
    <div className="font-sans bg-white text-slate-900 dark:bg-slate-900 dark:text-slate-100 min-h-screen scroll-smooth">
      {/* HEADER */}
      <header
        className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 ${
          headerSolid ? "bg-white/80 dark:bg-slate-900/80 backdrop-blur-md shadow" : "bg-transparent"
        }`}
      >
        <div className="max-w-6xl mx-auto px-6 py-4 flex justify-between items-center">
          <a href="#home" className="text-xl font-bold text-indigo-600">Oybek Ilhomboyev</a>
      ...
      <main className="pt-20">
        ...
        <section id="home" className="max-w-6xl mx-auto px-6 py-32 flex flex-col lg:flex-row items-center gap-12 relative overflow-hidden">
          ...
          >
            ...
            {/* Glassmorphism framed card with floating animation */}
            <motion.div
              animate={{ y: [0, -10, 0] }}
              transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }}
              className="relative p-4 rounded-3xl bg-white/30 dark:bg-slate-800/30 backdrop-blur-xl border border-white/20 dark:border-white/6 shadow-2xl"
              style={{ minWidth: 224 }}
            >
              <img
                src={imgSrc}
                ...
                onError={(e) => {
                  // fallback to embedded SVG (safe — won't break build)
                  const el = e.currentTarget;
                  if (el && el.src !== FALLBACK_SRC) el.src = FALLBACK_SRC;
                }}
              />
            </motion.div>
          </motion.div>
        ...
        <Section id="about">
          <div className="max-w-4xl mx-auto px-6">
            ...
            <a href="/Oybek_Ilhomboyev_CV.pdf" download className="inline-block mt-6 px-4 py-2 bg-indigo-600 text-white rounded-lg shadow hover:bg-indigo-700">Download CV</a>
          </div>
        </Section>
        ...
        <Section id="contact">
          <div className="max-w-3xl mx-auto px-6 text-center">
            ...
            <a href="/Oybek_Ilhomboyev_CV.pdf" download className="inline-block mt-6 px-6 py-3 bg-white dark:bg-slate-800 text-indigo-600 rounded-lg shadow hover:bg-slate-100 dark:hover:bg-slate-700">
              Download CV
            </a>
          </div>
        </Section>
